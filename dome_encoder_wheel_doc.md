...
│                                                                      │
│   ┌─────────┐      ┌─────────────────────────────────────┐          │
│   │ Motor   │      │    SRMIE — Subsistema de Medição     │          │
│   │ de      │      │                                      │          │
│   │ Rotação │      │  ┌──────────┐    ┌──────────────┐   │          │
│   │ (Atua-  │      │  │ Encoder  │◄───│    Roda de   │   │          │
│   │ dor)    │      │  │ Rotativo │    │  Medição     │   │          │
│   └────┬────┘      │  │  (Eixo   │    │ (Elastômero/ │   │          │
│        │           │  │  Direto) │    │  Poliuretano)│   │          │
│        │           │  └──────────┘    └──────┬───────┘   │          │
│        ▼           │                         │           │          │
│   ┌─────────┐      │              ┌──────────▼──────────┐│          │
│   │  Trilho │◄─────┤──────────────│  Braço Pantográfico ││          │
│   │Circular │      │              │ (Mola de Compressão)││          │
│   └─────────┘      │              └─────────────────────┘│          │
│                    └─────────────────────────────────────┘          │
│                                                                      │
│   Sinal Digital: Quadrature A/B/Z ──► CLP / Controlador de Cúpula  │
└──────────────────────────────────────────────────────────────────────┘
```
...
$$\theta_{res} = \frac{360°}{N_{PPR} \cdot \left(\frac{D_{trilho}}{D_{roda}}\right)}$$

Onde:

- $\theta_{res}$ — Resolução angular por pulso do encoder \[graus\]
- $N_{PPR}$ — Número de pulsos por revolução do encoder \[pulsos/rev\]
- $D_{trilho}$ — Diâmetro efetivo do trilho circular da cúpula \[mm\]
- $D_{roda}$ — Diâmetro externo da roda de medição \[mm\]

**Forma equivalente** em termos de perímetro:

$$\theta_{res} = \frac{P_{roda}}{P_{trilho}} \cdot \frac{360°}{N_{PPR}}$$

Onde $P_{roda} = \pi \cdot D_{roda}$ e $P_{trilho} = \pi \cdot D_{trilho}$.

**Resolução em minutos de arco:**

$$\theta_{res}['] = \theta_{res}[°] \times 60$$

**Com decodificação em quadratura (×4):**

$$\theta_{res,Q4} = \frac{\theta_{res}}{4}$$
...
| Cenário | $D_{trilho}$ | $D_{roda}$ | PPR Encoder | Resolução (×1) | Resolução (×4 Quad.) |
|---|---|---|---|---|---|
...
> **Recomendação de projeto:** Para rastreamento de objetos astronômicos com erro de apontamento < 0,1°, é suficiente um encoder de 1.000 PPR com roda de Ø100 mm em cúpulas até 6 m. Para precisão astrométrica superior (< 0,01°), use encoder ≥ 2.500 PPR com decodificação ×4.
...
$$\Delta\theta = \theta_{real} \cdot \alpha_{PU} \cdot \Delta T$$

Para poliuretano: $\alpha_{PU} \approx 150 \times 10^{-6}\ \text{°C}^{-1}$ (coeficiente de expansão volumétrica → coeficiente linear $\approx 50 \times 10^{-6}\ \text{°C}^{-1}$).

Para uma variação de temperatura de $\Delta T = 30°C$ (típico ambiente de cúpula, –5°C a +45°C):

$$\frac{\Delta D_{roda}}{D_{roda}} = 50 \times 10^{-6} \times 30 = 0,15\%$$

Erro escalar resultante: **0,15%** do ângulo lido. Para uma volta completa: $360° \times 0,0015 = 0,54°$ de erro total acumulado — aceitável para a maioria das aplicações de cúpula, mitigável por calibração com referência absoluta (canal Z ou sensor home).
...
A força de contato $F_{mola}$ deve ser suficiente para:
...
$$F_{min} = \frac{m_{cúpula} \cdot a_{max} \cdot D_{roda}}{2 \cdot D_{trilho} \cdot \mu}$$

Para cúpula de $m = 500\ kg$, aceleração máxima $a_{max} = 0,05\ m/s^2$, $\mu_{PU/aço} = 0,5$:

$$F_{min} = \frac{500 \times 0,05 \times 0,1}{2 \times 5 \times 0,5} = \frac{2,5}{5} = \mathbf{0,5\ N}$$
...
Onde $L$ = largura de contato \[m\], $R^* = R_{roda}$ (trilho plano → $R_{trilho}^{-1} \approx 0$), $E^*$ = módulo reduzido PU/aço.

Para PU Shore 70A: $E_{PU} \approx 5–10\ MPa$. A pressão de contato resultante fica na faixa **0,1–0,5 MPa** — muito abaixo do limite de esmagamento do PU (10–30 MPa), garantindo vida útil de > 5 anos em operação contínua.
...
$$K_{cal} = \frac{N_{pulsos\_medidos}}{360°}\ [\text{pulsos/grau}]$$
...
4. Registrar $N_{pulsos\_medidos}$.
5. Calcular o fator de calibração:

$$K_{cal} = \frac{N_{pulsos\_medidos}}{360}$$

6. Configurar $K_{cal}$ no firmware/CLP como divisor de escala.
7. Comparar com o valor teórico $K_{teorico} = \frac{N_{PPR} \cdot D_{trilho}}{D_{roda}}$. A diferença aceitável é < 0,5% (indica desvio de diâmetro de fabricação).

**Método 2 — Referência de Azimute (alta precisão)**

1. Usar bússola náutica digital compensada (precisão 0,5°) ou clinômetro de azimute para determinar dois azimutas de referência $Az_1$ e $Az_2$, separados por ângulo conhecido $\Delta Az$.
2. Mover a cúpula de $Az_1$ para $Az_2$ e registrar $\Delta N_{pulsos}$.
3. $K_{cal} = \frac{\Delta N_{pulsos}}{\Delta Az}$.
...
## Apêndice A — Diagrama de Blocos do Sistema de Controle

```
┌───────────────────────────────────────────────────────────────────────────────┐
│                        SISTEMA DE CONTROLE DA CÚPULA                         │
│                                                                               │
│  ┌─────────────────┐    Pulsos A/B/Z     ┌──────────────────────────────┐    │
│  │  ENCODER SRMIE  │────────────────────►│  MÓDULO HSC (High-Speed      │    │
│  │  (HTL 24V RS422)│    (Par trançado    │  Counter) do CLP              │    │
│  └─────────────────┘     blindado)       │  ou controlador dedicado      │    │
│                                          └──────────────┬───────────────┘    │
│  ┌─────────────────┐    Sinal Digital    ┌──────────────▼───────────────┐    │
│  │ SENSOR HOME     │────────────────────►│  CLP INDUSTRIAL / SBC        │    │
│  │ (Indutivo IP67) │                     │  (Arduino Mega / Raspberry    │    │
│  └─────────────────┘                     │  Pi + INDI / ASCOM / SW)     │    │
│                                          └──────────────┬───────────────┘    │
│                                                         │                    │
│                                          ┌──────────────▼───────────────┐    │
│                                          │  MOTOR DE ACIONAMENTO DA     │    │
│                                          │  CÚPULA (malha fechada)      │    │
│                                          │  Inversor + Motor CC/BLDC    │    │
│                                          └──────────────────────────────┘    │
│                                                                               │
│  Interface ASCOM/INDI para software astronômico: Stellarium, Cartes du Ciel, │
│  Maxim DL, KStars, TheSkyX                                                   │
└───────────────────────────────────────────────────────────────────────────────┘
```
