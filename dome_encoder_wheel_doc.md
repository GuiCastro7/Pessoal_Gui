...
```mermaid
flowchart TD
    PIVO["📍 Ponto de Pivô<br>(Fixação Rígida no Chassi Estático da Cúpula)"]
    PARAFUSO["🔩 Parafuso M8 c/ Bucha de Nylon<br>(Articulação com Folga Zero)"]
    
    subgraph BRACO_SET["Conjunto Braço Articulado"]
        BRACO["🦾 Braço Articulado<br>(Perfil de Alumínio 40×20 mm)"]
        MOLA["🌀 Mola de Compressão Calibrada<br>(Pré-carga Ajustável: 15–35 N)"]
        SLOT["📏 Slot de Ajuste de Curso<br>(Compensação Radial: ±8 a ±15 mm)"]
        
        BRACO --- MOLA
        BRACO --- SLOT
    end

    SUPORTE["🗜️ Suporte Usinado em Alumínio 6061<br>(Alojamento Coaxial de Rolamentos)"]
    
    subgraph MEASURE_HEAD["Cabeçote de Medição"]
        ENC["🧭 Encoder Rotativo Industrial<br>(Eixo Sólido / HTL 24V RS422)"]
        RODA["⚙️ Roda de Medição Métrica Ø100 mm<br>(Poliuretano Shore 70A)"]
        
        ENC <===>|Acoplamento Rígido Sem Folga| RODA
    end

    TRILHO["⭕ Trilho Circular da Cúpula<br>(Superfície de Rolamento Móvel)"]

    PIVO --> PARAFUSO --> BRACO
    BRACO --> SUPORTE
    SUPORTE --> MEASURE_HEAD
    RODA ===|Contato Direto Puro por Fricção| TRILHO
```
...
> - [ ] **Excentricidade do trilho:** medir com relógio comparador ao longo de 360° (tolerância máxima recomendada: $\pm 5\text{ mm}$).
> - [ ] **Emendas do trilho:** limar e nivelar soldas e juntas (desnível máximo tolerado: $0{,}5\text{ mm}$).
> - [ ] **Limpeza da pista:** limpar a superfície de contato com pano seco ou desengraxante biodegradável.
> - [ ] **Varredura de percurso:** certificar que não há parafusos, cabos ou obstáculos na trajetória da roda de medição.
> - [ ] **Ponto de ancoragem estático:** definir a posição de fixação do suporte no chassi estacionário da cúpula.

**Medição de excentricidade:** Posicionar relógio comparador apontado radialmente para o trilho e girar a cúpula manualmente 360°. Registrar a variação total de indicação (TIR — Total Indicator Runout). O curso de compensação do braço pantográfico deve ser $\ge \text{TIR} + 20\%$ de margem.
...
2. **Alinhamento axial:** O eixo da roda de medição deve ser **paralelo ao eixo de rotação da cúpula** (vertical). Usar prumo ou nível de bolha. Tolerância: $\le 0{,}5^\circ$ de inclinação.
3. **Perpendicularidade ao trilho:** O plano da roda deve ser perpendicular à superfície de contato do trilho (lateral ou inferior, conforme projeto). Verificar com esquadro de precisão. Tolerância: $\le 1^\circ$.
4. **Torque de fixação:** Parafusos M8 classe 8.8 $\rightarrow 22\text{ N}\cdot\text{m}$ com trava rosca (Loctite 243).
...
2. Rosquear o encoder ao suporte usinado, verificando que o eixo do encoder e o eixo da roda são **coaxiais** (tolerância: $< 0{,}1\text{ mm}$ de excentricidade).
...
4. Roteamento do cabo: curvar suavemente (raio mínimo: $10\times$ o diâmetro do cabo); usar conduíte corrugado flexível.
...
$$K_{\text{cal}} = \frac{N_{\text{pulsos medidos}}}{360^\circ}\quad [\text{pulsos/grau}]$$
...
4. Registrar $N_{\text{pulsos medidos}}$.
5. Calcular o fator de calibração:

$$K_{\text{cal}} = \frac{N_{\text{pulsos medidos}}}{360}$$
...
O canal Z do encoder incremental ou um sensor de posição home (indutivo IP67 no trilho) deve ser utilizado para **reset periódico do contador** e eliminação de deriva:

```mermaid
flowchart TD
    START["🚀 Energização do Sistema"] --> HOMING_CMD["Rotina de Homing Automático Iniciada"]
    HOMING_CMD --> ROT["Giro em Baixa Velocidade da Cúpula (Slew Rate: ~1°/s)"]
    ROT --> DETECT{"Sensor Home / Canal Z<br>Detectado?"}
    DETECT -- Não --> ROT
    DETECT -- Sim --> RESET["Zerar Contador de Pulsos<br>(Posição Angular Absoluta = 0,000°)"]
    RESET --> READY["Pronto para Operação / Rastreamento Sideral"]
    
    subgraph COMP["Mapeamento e Compensação de Excentricidade"]
        LUT["Tabela de Lookup (LUT) no CLP<br>Correções $\\Delta\\theta$ gravadas a cada 10°"]
        READY --> TRACK["Operação de Rastreamento"]
        LUT -.->|Interpolação de Correção| TRACK
    end
```
...
- Pré-carga medida (N)
- Fator de calibração $K_{\text{cal}}$ verificado
